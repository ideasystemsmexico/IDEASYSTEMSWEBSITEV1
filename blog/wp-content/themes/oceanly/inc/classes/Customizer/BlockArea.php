...
abstract class BlockArea extends Customizer {
	...
	public function pattern_blocks_choices() {
		...
		$pattern_blocks_choices = array( 0 => esc_html__( 'Select a block', 'oceanly' ) );
	...
	public function block_area_description() {
		return wp_kses(
			sprintf(
				...
				__( 'This is the content of the block area. You can create or edit the block area in the <a href="%s" target="_blank">Pattern Blocks Manager (opens in a new window)</a>.<br>After creating the pattern block, you may need to refresh this customizer page and then select the newly created block.<br>The selected block content will appear on the block area.', 'oceanly' ),
